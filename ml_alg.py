...
def arrange_data(vec_files, rot_files, nb):
    print 'Vec File ML : ' + str(vec_files)
    ...
    test_pred_df.to_csv('/media/axel/DATA/Axel/Documents/DATASET/pred_val_bis_61.csv', index=False, header=False)
    print('pred_val DONE')
    ...
    all_vec = '/media/axel/DATA/Axel/Documents/DATASET/vecShuf_' + str(nb) + '.csv'
    for vecFile in vec_files :
        # one_vec=[]
        tmpList = pd.read_csv(vecFile,header=None, low_memory=True)
        for index, row in tmpList.iterrows():
            vecList.append(row.tolist())
        print(vecFile + "Done")
        print 'VECLIST SIZE : ' + str(len(vecList))
        print 'len vecList[i] '+ str(len(vecList[0]))

        tmpList=[]
        # allVecListLength.append(len(one_vec))
        # vecListLength += len(one_vec)

    print 'VECLIST SIZE : ' + str(len(vecList))
    # print 'VECLIST SIZE : ' + str(vecListLength)

    failed = []
    j = 0
    for i in range (1,len(vecList)):
    # for i in range (1,vecListLength-1):
        # print str(allVecListLength[i]) + " and : " + str(allVecListLength[i-1])
        if len(vecList[i]) == len(vecList[i-1]):
        # if allVecListLength[i] == allVecListLength[i-1]:
            pass
        else :
            print 'OOOOOUUUUUUPS !'
            failed.append(i)
    ...
    all_rot = '/media/axel/DATA/Axel/Documents/DATASET/rotShuf_' + str(nb) + '.csv'
    ...
    shuf = '/media/axel/DATA/Axel/Documents/DATASET/shuf_' + str(nb) + '.csv'
    if len(training_data) == len(training_labels) :
            index_shuf = range(len(training_data))
    else :
    ...
    training_size = 70*len(training_data)//100
    # training_size = 70*vecListLength//100

    for i in index_shuf:
        dataShuf.append(training_data[i])
        labelShuf.append(training_labels[i])
    ...
    # test_pred_df = pd.DataFrame(test_labels)
    # pred_csv = '/media/axel/DATA/Axel/Documents/DATASET/pred_val_' + str(nb) + '.csv'
...
def learn(training_data, training_labels, test_data, test_labels, nb):
    ...
    for j in x :
        ...
        rfr_pred_path = '/media/axel/DATA/Axel/Documents/DATASET/rfr_pred_bis_61' + str(nb)  + '_' + str(j) + '.csv'
